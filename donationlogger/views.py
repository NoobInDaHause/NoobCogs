...
class DonationLoggerSetupView(nu.NoobView):
    ...
    async def update_embed_setup(self) -> discord.Embed:
        ...
        ar2 = "".join(
            [
                f"` - ` **{nu.cf.humanize_number(int(k))}**: "
                f"{nu.cf.humanize_list([role.mention for role in v])}\n"
        ...
        man_r2 = (
            nu.cf.humanize_list([r.mention for r in self.manager_roles])
...
class BankNameModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction[nu.Red]) -> None:
        await interaction.response.defer()
